...
async function getItem(request, response) {
    ...
    const result = await invoice_item_1.default.getItem(id, request.user);
...
async function list(request, response) {
    ...
    const query = invoice_item_1.Model.find()
        .where('company_id')
...
async function create(request, response) {
    ...
    delete newItem._id;
    try {
        const result = await invoice_item_1.default.create({
            item: newItem,
            offlineId,
            user: request.user,
        });
...
async function update(request, response) {
    const item = request.body;
    const newItem = Object.assign({}, item);
    const result = await invoice_item_1.default.update({
        item: newItem,
        user: request.user,
    });
    response.status(200).json(result);
...
async function remove(request, response) {
    const result = await invoice_item_1.default.remove({
        item: request.body,
        user: request.user,
    });
    response.status(200).json(result);
